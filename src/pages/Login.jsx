  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!isPartner && otpSent) {
        ...
        const partnerEmail = partnerId;
        const partnerData = mockPartnerDataService.getPartnerData(partnerEmail);

        if (!partnerData) {
          showError("Partner not found. Please check your email.");
          return;
        }
        ...
        localStorage.setItem(
          "accessToken",
          `mock_partner_token_${partnerData.id}`
        );
        localStorage.setItem(
          "refreshToken",
          `mock_partner_refresh_token_${partnerData.id}`
        );
        localStorage.setItem("user", JSON.stringify(partnerUser));

        setUser(partnerUser);

        showSuccess(`Login successful! Welcome ${partnerData.name}`);
        setLoginSuccess(true);
